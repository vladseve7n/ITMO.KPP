...
def convert_images_to_video(images_path, videoname, fps=30, audio_path=None):
    ...
    images_names = sorted(glob.glob(os.path.join(images_path, '*[0-9]*.png')),key=lambda x: int(x.split('/')[-1].split('.')[0]))
